            ...
            <?php else: ?>
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 16px;">
                    <?php foreach ($payment_history as $p): ?>
                        <div style="background: #fff; padding: 20px; border-radius: 12px; border: 1px solid var(--border); box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.05); transition: transform 0.2s;" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='none'">
                            <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 12px;">
                                <div>
                                    ...
                                    <div style="font-size: 12px; color: var(--text-gray); text-transform: uppercase; font-weight: 700; margin-top: 4px;">
                                        <?php echo htmlspecialchars($p['bill_type']); ?> Bill • <?php echo htmlspecialchars($p['month'] ?? 'N/A'); ?>
                                    </div>
                                </div>
                                <div style="text-align: right;">
                                    <span style="background: rgba(16,185,129,0.1); color: #10B981; padding: 6px 12px; border-radius: 8px; font-size: 11px; font-weight: 700; border: 1px solid #A7F3D0;"><i class='bx bx-check' ></i> <?php echo htmlspecialchars($p['payment_mode']); ?></span>
                                </div>
                            </div>
                            <div style="font-size: 12px; color: var(--text-gray); display: flex; align-items: center; justify-content: space-between; border-top: 1px dashed #CBD5E1; padding-top: 16px; margin-top: 16px;">
                                <span style="display: flex; align-items: center; gap: 6px;"><i class='bx bx-calendar' style="font-size: 14px;"></i> <?php echo date('d M Y, h:i A', strtotime($p['payment_date'] . ' ' . $p['payment_time'])); ?></span>
                                <?php if ($p['admin_name']): ?>
                                    <span style="display: flex; align-items: center; gap: 6px; font-weight: 500;"><i class='bx bx-user' style="font-size: 14px;"></i><?php echo htmlspecialchars($p['admin_name']); ?></span>
                                <?php endif; ?>
                            </div>
                        </div>
                    <?php endforeach; ?>
