...
<head>
    ...
    <style>
        ...
        .history-item::before {
            ...
            bottom: 0; /* Stretch perfectly to bottom of padding */
            width: 2px;
            background: #E2E8F0;
            z-index: 0;
        }
        ...
        @media (max-width: 768px) {
            ...
            .reminder-card { padding: 20px !important; border-radius: 16px !important; }
...
<main class="main">
    ...
    <div class="reminders-custom-grid animate-up">
        
        <!-- LEFT COLUMN: Floated Cards (No Panel Background) -->
        <div class="left-col" style="display: flex; flex-direction: column; gap: 20px;">
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 0 4px; margin-bottom: 8px;">
                <h2 style="font-size: 20px; font-weight: 800; color: #0F172A; margin: 0;">Pending Dues & Reminders</h2>
                <span style="background: #FEE2E2; color: #EF4444; font-weight: 700; padding: 6px 14px; border-radius: 8px; font-size: 13px;">
                    <?php echo count($dues); ?> Unpaid Bills
                </span>
            </div>
            
            <?php if(empty($dues)): ?>
                <div style="text-align: center; padding: 60px; color: var(--text-gray); background: #ffffff; border-radius: 20px; border: 1px dashed #CBD5E1;">
                ...
                <div class="reminder-card <?php echo $isDisabled ? 'disabled' : ''; ?>">
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; flex-wrap: wrap; gap: 16px;">
                        <div style="display: flex; align-items: center; gap: 16px;">
                            <div style="width: 56px; height: 56px; border-radius: 16px; display: flex; align-items: center; justify-content: center; font-size: 26px; background: <?php echo $d['type'] == 'Rent' ? 'rgba(98,75,255,0.1); color: #624BFF;' : 'rgba(245,158,11,0.1); color: #F59E0B;'; ?>">
                            ...
                            <div>
                                <h3 style="font-size: 18px; font-weight: 800; color: var(--text-dark); margin: 0 0 6px 0; display: flex; align-items: center; gap: 8px;">
                                    ...
                                    <span style="font-size: 11px; font-weight: 700; padding: 4px 10px; border-radius: 8px; <?php echo $d['type'] == 'Rent' ? 'background: #EEF2FF; color: #4F46E5;' : 'background: #FFF7ED; color: #F59E0B;'; ?>">
                        ...
                        <div style="text-align: right; background: #F8FAFC; padding: 12px 20px; border-radius: 14px; border: 1px solid #F1F5F9;">
                            <div style="font-size: 11px; color: var(--text-gray); font-weight: 700; margin-bottom: 2px; letter-spacing: 0.5px; text-transform: uppercase;">Due Amount</div>
                            <div style="font-size: 20px; font-weight: 800; color: #EF4444;">₹<?php echo number_format($d['type'] == 'Rent' ? $d['rent_amount'] : $d['total_amount'], 2); ?></div>
                        </div>
                    </div>
                    ...
                    <div class="action-row">
                        ...
                        <div style="display: flex; gap: 12px;">
                            <?php if ($isDisabled): ?>
                                <a href="manage-reminders.php?action=toggle&id=<?php echo $d['id']; ?>&type=<?php echo $d['type']; ?>&status=Enabled" class="btn-outline" style="padding: 10px 18px; font-size: 13px; border-radius: 12px; color: #10B981; border-color: #10B981; text-decoration: none; display: flex; align-items: center; gap: 6px; font-weight: 700;">
                                    <i class='bx bx-play-circle' style="font-size: 18px;"></i> Resume
                                </a>
                            <?php else: ?>
                                <a href="manage-reminders.php?action=toggle&id=<?php echo $d['id']; ?>&type=<?php echo $d['type']; ?>&status=Disabled" class="btn-outline" style="padding: 10px 18px; font-size: 13px; border-radius: 12px; text-decoration: none; display: flex; align-items: center; gap: 6px; font-weight: 700;">
                                    <i class='bx bx-pause-circle' style="font-size: 18px;"></i> Pause
                                </a>
                            <?php endif; ?>
                            
                            <a href="manage-reminders.php?action=remind&id=<?php echo $d['id']; ?>&type=<?php echo $d['type']; ?>" class="btn-primary" style="padding: 10px 24px; font-size: 13px; border-radius: 12px; text-decoration: none; display: flex; align-items: center; gap: 6px; font-weight: 700; <?php echo $isDisabled ? 'opacity: 0.5; pointer-events: none;' : ''; ?>">
                                <i class='bx bx-send' style="font-size: 18px;"></i> Send Now
                            </a>
                        </div>
                    </div>
                </div>
        ...
        <!-- RIGHT COLUMN: Solid Panel -->
        <div class="right-col">
            <div class="panel" style="position: sticky; top: 100px;">
                <div class="panel-header" style="margin-bottom: 28px;">
                    <h2 style="font-size: 18px; font-weight: 800; color: #0F172A; display: flex; align-items: center; gap: 10px;">
                        <i class='bx bx-history' style="color: var(--primary-purple); background: rgba(98, 75, 255, 0.1); padding: 8px; border-radius: 10px; font-size: 20px;"></i>
                        Reminders History
                    </h2>
                </div>
                <div>
                    <?php while($h = mysqli_fetch_assoc($history)): ?>
                        <div class="history-item">
                            ...
                            <div style="flex: 1; padding-top: 2px;">
                                ...
                                <div style="font-size: 13px; color: var(--text-gray); font-weight: 600; margin-bottom: 8px;">
                                    <?php echo $h['bill_type']; ?> Reminder (<?php echo $h['month']; ?>)
                                </div>
                                <div style="display: inline-flex; align-items: center; font-size: 11px; color: var(--text-gray); font-weight: 700; background: #F8FAFC; border: 1px solid #E2E8F0; padding: 4px 10px; border-radius: 6px;">
                                    <i class='bx bx-time-five' style="margin-right: 4px; font-size: 14px;"></i> <?php echo date('M d', strtotime($h['sent_at'])); ?> • <?php echo date('g:i A', strtotime($h['sent_at'])); ?>
                                </div>
                            </div>
                        </div>
                    <?php endwhile; ?>
                ...
                <div style="margin-top: 32px; padding: 16px; background: #F8FAFC; border-radius: 12px; border: 1px dashed #CBD5E1; text-align: center;">
                    <p style="font-size: 12px; color: var(--text-gray); font-weight: 600; margin: 0; display: flex; align-items: center; justify-content: center; gap: 8px;">
                        ...
                        Auto-Reminders run after the 20th of each month.
                    </p>
                </div>
            </div>
        </div>
