...
<div class="animate-up" style="padding: 0 16px 15px 16px;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px;">
        <div style="display: flex; align-items: center; gap: 2px; border: 1px solid var(--border); padding: 4px 12px; border-radius: 8px; font-size: 12px; font-weight: 600; color: var(--text-dark); background: var(--white);">
        ...
        <button style="border: 1px solid var(--border); background: var(--white); padding: 8px 12px; border-radius: 8px; font-size: 12px; font-weight: 600; color: var(--text-dark); display: flex; align-items: center; gap: 2px; cursor: pointer;">
    ...
    <div style="display: flex; flex-direction: column; background: var(--white); border-radius: 16px; border: 1px solid var(--border); overflow: hidden; margin-bottom: 24px;">
        ...
        $mobile_aggregates = [];
        foreach($all_mobile as $t) {
            $m = $t['month'];
            if(!isset($mobile_aggregates[$m])) {
                $mobile_aggregates[$m] = [
                    'item_type' => 'aggregate',
                    'month' => $m,
                    'amount' => 0,
            ...
            elseif($st == 'paid') $mobile_aggregates[$m]['has_paid'] = true;
        }

        foreach($mobile_aggregates as &$agg) {
            $agg['status'] = 'Unpaid';
        ...
        foreach ($all_mobile as $idx => $t): 
            ...
            if ($t['item_type'] === 'aggregate'):
                ...
                $dataType = 'all-aggregate';
                $isPending = ($t['status'] === 'Unpaid' || $t['status'] === 'Partial');
        ?>
    ...
    (function() {
        ...
        function applyFiltersAndPagination() {
            const visibleItems = items.filter(it => {
                ...
                let typeMatch = false;
                if (currentType === 'all') {
                    typeMatch = (type === 'all-aggregate');
                } else {
                    typeMatch = (type === currentType);
                }
    ...
    <div class="pending-payment-notice">
        ...
        <button onclick="openPaymentModal(<?php echo $total_due; ?>, 'Total Outstanding Balance', 'total')" style="width: 100%; background: #624BFF; color: white; border: none; border-radius: 10px; padding: 12px; font-size: 13px; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 2px; cursor: pointer;">
