...
$stmt = mysqli_prepare($conn, "SELECT 
    ...
    IFNULL(SUM(CASE WHEN rent_status = 'Due' OR (rent_status = '' AND status = 'Due') OR (status = 'Due' AND rent_status = 'Due') THEN (rent_amount + maintenance) ELSE 0 END), 0) as rent_portion_total 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_payment_notif'])) {
    if (!isset($_POST['csrf']) || !hash_equals($_SESSION['csrf'], $_POST['csrf'])) {
        ...
        if ($payment_method === 'UPI' && strpos($tr_id, 'SYS_') === 0) {
...
<head>
    ...
    <style>
        ...
        .reminder-banner .btn-pay-now { background: var(--white); color: #FF4B6B; padding: 12px 24px; border-radius: 14px; font-weight: 700; font-size: 15px; border: none; cursor: pointer; display: flex; align-items: center; gap: 2px; z-index: 2; transition: all 0.2s; text-decoration: none; box-shadow: 0 4px 15px rgba(0,0,0,0.1);}
        ...
        .page-btn { width: 40px; height: 40px; border-radius: 12px; display: flex; align-items: center; justify-content: center; background: var(--white); border: 1px solid var(--border); color: var(--text-gray); font-size: 13px; font-weight: 600; text-decoration: none; transition: 0.2s; }
...
<div class="app-container">
    ...
    <main class="main-content">
        <!-- 1. EXCLUSIVE MOBILE VIEW CODE (Isolated in views/mobile/payment-history_mobile.php) -->
        <div class="mobile-view-wrapper">
            <?php include __DIR__ . '/views/mobile/payment-history_mobile.php'; ?>
        </div>

        <!-- 2. EXCLUSIVE DESKTOP VIEW CODE (Isolated in views/desktop/payment-history_desktop.php) -->
        <div class="desktop-view-wrapper">
            <?php include __DIR__ . '/views/desktop/payment-history_desktop.php'; ?>
        </div>
</main>
