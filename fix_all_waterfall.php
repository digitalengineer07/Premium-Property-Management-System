...
$files = [
    ...
    'renter/views/mobile/my-payments_mobile.php',
...
foreach ($files as $file) {
    ...
    $content = str_replace(
        "SELECT e.id, e.month, e.rent_amount, e.maintenance, e.dues, e.extra_charges, e.extra_charges_desc, COALESCE(NULLIF(e.rent_status, ''), e.status) as status",
        "SELECT e.id, e.month, e.amount, e.rent_amount, e.maintenance, e.dues, e.extra_charges, e.extra_charges_desc, COALESCE(NULLIF(e.rent_status, ''), e.status) as status",
        $content
    );
    $content = str_replace(
    ...
    if (strpos($content, '$paid_towards_rent = max(0, $total_paid - $elec_amt);') === false) {
...
PHP;
    ...
    // 5. Replace max(0, amount - total_paid) for rent
